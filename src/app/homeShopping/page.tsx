...
function homeShopping() {
  ...
  const [shopCdsModalValue, setShopCdsModalValue] = useState<any>("");

  const [selectedChannels, setSelectedChannels] = useState<string[]>([]);
  const [selectedChannelsView, setSelectedChannelsView] = useState<string[]>(
    []
  );
  const [selectedChannelModalValue, setSelectedChannelModalValue] = useState<
    string[]
  >([]);
  ...
  const handleChannelChange = (event: any) => {
    ...
    if (id === "channelall") {
      ...
      if (selectedChannelsView.includes("전체")) {
        setSelectedChannelsView(
          selectedChannelsView.filter((i: string) => i != "전체")
        );
        setSelectedChannels(
          selectedChannels.filter((i: string) => i != "전체")
        );
      }
      ...
      if (checked) {
        ...
        setSelectedChannelsView((prevSelectedChannelsView: any) => [
        ...
        setSelectedChannelsView((prevSelectedChannels: any[]) =>
          prevSelectedChannels.filter((channel: any) => channel !== id)
        );
      }
    }
  ...
  const handleShopCdChange = (event: any) => {
    ...
    if (id === "전체") {
      ...
      if (shopCdsView.includes("전체")) {
        setShopCdsView(shopCdsView.filter((i: string) => i !== "전체"));
        setShopCds(shopCds.filter((i: string) => i !== "전체"));
      }
      if (checked) {
        setShopCds((prevShopCds: any) => [...prevShopCds, id]);
        setShopCdsView((prevShopCds: any) => [...prevShopCds, id]);
      } else {
        setShopCds((prevShopCds: any[]) =>
          prevShopCds.filter((shopCd: any) => shopCd !== id)
        );
        setShopCdsView((prevShopCds: any[]) =>
          prevShopCds.filter((shopCd: any) => shopCd !== id)
        );
      }
    }
  ...
  const fetchData = async () => {
    try {
      ...
      axios
        ...
        .then(
          (res: {
            data: { recentData: any; percentData: any; searchData: any };
          }) => {
  ...
  const formattedDate = `${today.getFullYear()}. 0${
    today.getMonth() + 1
  }. ${today.getDate()}(${todayLabel}) 05:00:00`;
  ...
  const channels = Object.entries(shopName).map(([id, label]) => ({
    id,
    label,
  }));
  ...
  const cateOpen = (e: any, kind: string) => {
    e.preventDefault();
    setGoodsType(kind);
    console.log("shopCds", shopCdsView);

    setShopCdsModalValue(shopCdsView);
    setSelectedChannelModalValue(selectedChannels);
    // setSelectedChannels([]);
    setIsModalOpen(true);
    setModalCateNm("");
    setStandardTime("");
  ...
  const shopOpen = (e: any, kind: string) => {
    ...
    // console.log("channel.id=", selectedChannels);
    // setSelectedChannels([channel.id]);
    setShopCdsModalValue(shopCdsView);

    console.log("shopCds1212==", shopCds);
    // setShopCdsModalValue(shopCds);

    setSelectedChannelModalValue([channel.id]);
  ...
  return (
    <div>
        ...
        <div className="btm_chart">
                  ...
                  <li>
                    ...
                    <div className="chart">
                      {category.cnt ? (
                        <a
                          href=""
                          onClick={(e: any) => cateOpen(e, category.kind)}
                          key={idx}
                        >
          ...
          <section className="search_box">
            ...
            <ul className="chart_box_h">
              {shopCategory.map(
                (
                  shopCate: { kind: string; cnt: string; percent: any },
                  idx: any
                ) => (
                  <li>
                    <h4>{shopCate.kind}</h4>
                    <div className="chart">
                      {shopCate.cnt ? (
                        <a
                          href=""
                          onClick={(e: any) => shopOpen(e, shopCate.kind)}
                          key={idx}
                        >
          ...
          <section className="search_box">
            <h3>시간대 별</h3>
            <ul className="chart_box_h">
              {standardCategorys.map(
                (
                  standardCategory: { kind: string; cnt: string; percent: any },
                  idx: any
                ) => {
                  return (
                    <li
                    ...
                    >
                      ...
                      <div className="chart">
                        {`${today.getHours().toString().padStart(2, "0")}시` ===
                          standardCategory.kind && <span>LIVE</span>}
                        {standardCategory.cnt ? (
                          <a
                            href=""
                            onClick={(e: any) =>
                              timeOpen(e, standardCategory.kind)
                            }
                            key={idx}
                          >
                            <div
                              style={{ width: `${standardCategory.percent}%` }}
                            >
          ...
          <section className="search_box">
            ...
            <ul className="chart_box_donut">
              <li className="chart_donut_wrap">
                {kindProds.map(
                  (kind: { cnt: string; kind: string }, idx: any) => {
                    return (
                      <h4>
                        {kind.cnt ? (
                          <a
                            href="#"
                            key={idx}
                            onClick={() => goodsTypeOpen(kind.kind)}
                          >
                            <span
                              style={
                                kind.kind === "일반"
      ...
      >
        <ProductList
          goodsType={goodsType}
          shopCdsModalValue={shopCdsModalValue}
          goodsTypeChart={goodsTypeChart}
          prodName={prodName}
          fromDate={startDate}
          toDate={endDate}
          shopCds={shopCds.includes("전체") ? [] : shopCds}
          cateNm={selectedChannelModalValue}
          modalCateNm={modalCateNm}
